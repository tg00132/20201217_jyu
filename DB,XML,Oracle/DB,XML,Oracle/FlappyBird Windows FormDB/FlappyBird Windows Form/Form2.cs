    ...
    public partial class RankingList : Form
    {
        ...
        private void writeLog(string contents)
        {
            string logContents = $"[{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")}] {contents}";
             //logContents라는 변수에 현재시각 + 내용을 출력하게함.

            listBox1.Items.Add(logContents); //새로운 것이 가장 위에 올라가는 방식

            listBox1.Items.Insert(0, logContents);  //새로운 것이 가장 위에 올라가는 방식
            data.printLog(logContents); // rank.txt에 logContents 내용을 불러온다.
        }
